...
function SectionHead({ number, label, title, copy }: { number: string; label: string; title: string; copy?: string }) {
  return <div className="reveal mb-12 grid min-w-0 gap-5 border-t pt-5 lg:grid-cols-[1fr_3fr]"><div className="eyebrow text-muted-foreground">{number} — {label}</div><div className="min-w-0"><h2 className="display-title max-w-4xl break-words text-[2.65rem] sm:text-6xl lg:text-8xl">{title}</h2>{copy&&<p className="mt-7 max-w-xl text-sm leading-7 text-muted-foreground">{copy}</p>}</div></div>;
}
...
function Standards() {
  ...
  return <section id="system" className="section-pad page-shell">
    <SectionHead number="03" label="Our standard" title="Consistency Is What Makes a Catalog Feel Designed." copy="Product accuracy comes first. We use the client reference as the source of truth, then apply controlled framing, camera language and presentation rules so every image feels like part of one deliberate catalog system."/>
    ...
    <div className="mt-24 grid items-start gap-8 border-t pt-8 lg:grid-cols-[minmax(0,1fr)_22rem]"><div className="min-w-0"><p className="eyebrow text-muted-foreground">Camera standard</p><h3 className="display-title mt-4 break-words text-[2.65rem] sm:text-5xl">Angles should explain the product.</h3><div className="mt-8 flex flex-wrap gap-2">{["Front","30–35°","Detail","In context"].map((name,i)=><Button key={name} size="sm" variant={camera===i?"primary":"outline"} onClick={()=>setCamera(i)}>{name}</Button>)}</div></div><div className="relative aspect-square w-full max-w-sm justify-self-center overflow-hidden bg-card lg:justify-self-end"><img src={cameraImages[camera]} alt={`A1-Oil-123 ${cameraNames[camera]} product view`} loading="lazy" width={1254} height={1254} className="size-full object-cover transition-all duration-500"/><span className="absolute bottom-3 left-3 bg-primary px-3 py-2 text-[9px] uppercase text-primary-foreground sm:bottom-4 sm:left-4 sm:text-[10px]">{camera===1?"Preferred when depth matters":"Controlled camera view"}</span></div></div>
  </section>;
}

function Explosion() {
  return <section className="section-pad overflow-hidden bg-secondary"><div className="page-shell text-center"><p className="eyebrow">05 — One to six</p><h2 className="display-title mx-auto mt-5 max-w-4xl break-words text-[2.65rem] sm:text-6xl lg:text-8xl">One Reference.<br/>Six Ways to Understand the Product.</h2><p className="mx-auto mt-6 max-w-lg text-sm text-muted-foreground">Build a complete product story instead of relying on a single photograph.</p>
...
function ProductPage() {
  const [selected,setSelected]=useState(0);
  const listingImages = [
    "/images/8436566807920_1.png",
    "/images/8436566807920_2.png",
    "/images/8436566807920_3.png",
    "/images/8436566807920_4.png",
    "/images/8436566807920_5.png",
  ];
  ...
  return <section className="section-pad bg-primary text-primary-foreground"><div className="page-shell"><SectionHead number="10" label="Listing experience" title="See the Difference Where It Matters."/>
    <div className="grid gap-8 bg-background p-4 text-foreground md:p-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(18rem,.65fr)] lg:gap-6"><div><div className="aspect-square overflow-hidden bg-card lg:mx-auto lg:h-[clamp(25rem,54vh,35rem)] lg:w-full lg:aspect-auto"><img src={listingImages[selected]} alt={`Natsbi Steamed Lamb cat food — ${listingImageNames[selected]} view`} loading="lazy" width={512} height={768} className="size-full object-contain"/></div><div className="mt-3 grid grid-cols-5 gap-2 lg:mx-auto lg:max-w-[44rem]">{listingImages.map((src,i)=><button key={src} onClick={()=>setSelected(i)} aria-label={`View ${listingImageNames[i]}`} className={`aspect-square overflow-hidden border-2 lg:max-h-24 ${selected===i?"border-signal":"border-transparent"}`}><img src={src} alt="" width={100} height={100} className="size-full object-contain"/></button>)}</div></div>
      <div className="flex flex-col justify-between py-2"><div><p className="eyebrow text-muted-foreground">Client product / SKU 8436566807920</p><h3 className="display-title mt-4 text-4xl xl:text-5xl">8436566807920</h3><p className="mt-2 text-sm text-muted-foreground">Natsbi Steamed / 43% Lamb / Complete food for cats</p><div className="mt-6 border-t py-4 text-sm leading-6">A gently steam-cooked, grain-free complete food for cats with 43% lamb, 80% moisture and taurine. Made with broth, carrot, parsnip and dried tomato, with chicory inulin, yeast products and seaweed. 80 g pouch.</div></div><Button onClick={openProjectForm}>Enquire about this product <ArrowRight size={15}/></Button></div></div>
  </div></section>;
}
...
function Index() {
  ...
  return <main id="top" className="overflow-clip"><Navigation/><ProjectFormDialog/>
    ...
    <section className="section-pad bg-secondary"><div className="page-shell"><SectionHead number="07" label="Product accuracy" title="Presentation Changes. The Product Alignment Remains Absolute." copy="Generic one-click image tools can drift from the real product—changing silhouettes, textures, logos or construction details. IMAGENMERCE uses a technology-assisted studio workflow shaped by human art direction, manual isolation and pixel-level finishing to keep the product aligned with the supplied reference."/><div className="grid items-start gap-10 lg:grid-cols-[24rem_minmax(0,22rem)] lg:justify-center"><Compare left="/images/product-accuracy-before.webp" right="/images/product-accuracy-after.png" leftLabel="Before" rightLabel="After" className="aspect-square w-full max-w-sm justify-self-center lg:max-w-none"/><div className="flex min-w-0 flex-col justify-between"><div><p className="eyebrow mb-4 text-muted-foreground">What we lock</p>{["Silhouette & proportions","Visible materials","Construction details","Recognizable textures","Product color","Logos & important markings","Cross-image consistency","Manual material isolation where required"].map(x=><div key={x} className="flex items-center gap-3 border-t py-4 text-sm"><Check size={15} className="shrink-0 text-signal"/>{x}</div>)}</div><div className="mt-8 border-l-2 border-signal pl-5"><p className="display-title break-words text-4xl">The reference image is the source of truth.</p><p className="mt-4 text-xs leading-6 text-muted-foreground">Backgrounds and camera presentation can change. Product markings, construction cues and identifiable details are checked against the original reference throughout the workflow.</p></div></div></div></div></section>
