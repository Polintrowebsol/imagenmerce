...
function SixSequence() {
  ...
  return <section id="sequence" className="relative bg-primary text-primary-foreground"><div className="sticky top-24 z-20 ml-auto hidden w-16 flex-col items-center gap-3 border-y border-primary-foreground/20 bg-primary/80 py-4 backdrop-blur md:flex">{sequence.map((s,i)=><a href={`#image-${i+1}`} key={s[0]} className={`text-[10px] transition-all ${active===i?"scale-125 text-accent":"opacity-35"}`}>{s[0]}</a>)}</div>
    {sequence.map((item,i)=><article key={item[0]} id={`image-${i+1}`} data-sequence data-index={i} className={`sequence-panel grid min-h-screen items-center gap-10 px-4 py-28 md:px-12 lg:grid-cols-[.8fr_1.2fr] ${active===i?"is-active":""}`}><div className="mx-auto max-w-xl"><p className="eyebrow text-accent">{item[0]} — {item[1]}</p><h2 className="display-title mt-6 text-6xl sm:text-7xl lg:text-8xl">{item[2]}</h2><p className="mt-7 max-w-md text-sm leading-7 text-primary-foreground/65">{item[3]}</p></div><div className="mx-auto aspect-square w-full max-w-3xl overflow-hidden bg-card"><img src={images[i]} alt={`RF-A1592 sideboard — ${imageNames[i]}`} loading="lazy" width={512} height={768} className="size-full object-cover"/></div></article>)}
  </section>;
}
