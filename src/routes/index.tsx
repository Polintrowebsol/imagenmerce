...
function Index() {
  ...
  return <main id="top" className="overflow-clip"><Navigation/><ProjectFormDialog/>
    <section className="page-shell flex flex-col justify-center pb-8 pt-20 sm:min-h-[86vh] sm:pb-10 sm:pt-32"><div className="grid min-w-0 items-center gap-5 sm:gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,46rem)]"><div className="order-2 min-w-0 max-w-[38rem] py-2 sm:order-1 sm:py-8 lg:justify-self-start"><p className="eyebrow text-signal">Product imaging studio / Art-directed systems</p><h1 className="display-title mt-4 break-words text-[2.35rem] sm:mt-6 sm:text-6xl lg:text-[4rem]">Turn One Product Photo Into A High-Converting Visual System.</h1><p className="mt-4 max-w-xl text-sm leading-6 text-muted-foreground sm:mt-7 sm:text-base sm:leading-8">Skip the cost and logistics of repeated commercial photoshoots. IMAGENMERCE is an art-directed product imaging studio that turns one clear reference photograph into a coordinated, marketplace-ready six-image visual system. For eligible, pre-scoped projects, production can be completed in as little as 48 hours.</p><div className="mt-8 flex flex-wrap gap-3"><Button onClick={openProjectForm}>Transform My Product <ArrowRight size={15}/></Button><Button variant="outline" asChild><a href={catalogEmailHref}>Discuss A Catalog <ArrowRight size={15}/></a></Button></div></div><Compare className="order-1 aspect-square w-full max-w-[46rem] min-h-0 justify-self-center sm:order-2 lg:justify-self-end"/></div></section>

    <ClientWork/>

    <section id="process" className="section-pad bg-primary text-primary-foreground"><div className="page-shell"><SectionHead number="02" label="Reference to ready-to-use" title={<><span className="lg:whitespace-nowrap">You send the product.</span><br/><span className="lg:whitespace-nowrap">We build the visual system around it.</span></>}/>
      <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,38rem)_minmax(0,1fr)] lg:justify-center"><div className="relative aspect-square w-full max-w-[38rem] justify-self-center overflow-hidden bg-card lg:justify-self-start"><img src={processSteps[processStep]![2]} alt={`RF-A1592 sideboard — ${processSteps[processStep]![0]}`} width={1536} height={1536} className={`size-full transition-all duration-700 ${processStep===0?"object-cover":"object-contain p-[6%]"}`}/>
